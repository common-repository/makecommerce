...
class Shipping {
    ...
    public function set_parcel_machine_meta( $post_id ) {
		...
		if ( isset( $_POST['_mk_machine_id'] ) ) {
			...
			if ( $_POST['_mk_machine_id'] != $current_parcel_machine || $update ) {
				...
				set_transient( '_set_parcel_machine_meta_transient', true, 5 );
    ...
    public function update_courier_meta ( $post_id ) {
		...
		if ( is_array( $method ) && count( $method ) > 0 ) {
			...
			if ( array_values( $method )[0] == "courier_smartpost"
				...
				set_transient( '_update_courier_meta_transient', true, 5 );
		...
		foreach ( $post_ids as $post_id ) {
			...
			foreach ( $shipping_methods as $shipping_method ) {
				...
				if ( empty( $shipping_request['credentials'][$carrier_uc] ) ) {
					...
					if ( empty( $shipping_request['credentials'][$carrier_uc] ) && empty( $transport_class->settings['use_mk_contract'] ) ) {
						$order->update_meta_data(
							'_parcel_machine_error',
							__( 'Request failed: missing credentials', 'wc_makecommerce_domain' )
						);
						$order->save();
						continue;
					}
				...
				if ( !$sr_order ) {
					$order->update_meta_data(
						'_parcel_machine_error',
						__( 'Request failed: invalid sender or destination data', 'wc_makecommerce_domain' )
					);
					$order->save();
					continue;
				}
    ...
    public function set_shipping_request_credentials( $carrier_uc, $transport_class, $shipping_request ) {
        ...
        // Check if any other credentials are added
        // If not, then leave empty to skip registering
        if ( !empty( $shipping_request['credentials'][$carrier_uc] ) ) {
	...
	public function set_sr_order_data( 
		...
		// If every value in sender except for country is empty, stop request
		// Country is a list, cant be empty after saving
		if ( count( array_filter( $sender ) ) <= 1 ) {
			return;
		}
	...
	public function invalid_api_key_notification() {
		...
		$dismiss_href = esc_url( add_query_arg( $_GET ) );
