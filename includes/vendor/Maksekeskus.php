...
class Maksekeskus
{
   ...
   /**
    ...
    {
        $request_body = $this->addMetaData($request_body);
   ...
   /**
    ...
    public function getLabelFormats ()
    {
        ...
        if (in_array($response->code, array(200, 201))) {
            ...
            throw new MKException('Could not get parcel label formats. Response ('.$response->code.'): '.$response->raw_body, $response->body->code);
        }
    }

    /**
     * get LP Express parcel presets
     *
     * @throws MKException if failed to get presets list
     * @return array List of templates with values and labels
     */
    public function getLPExpressPresets ()
    {
        $response = $this->makeGetRequest('/v1/shipments/presets/LP_EXPRESS_LT?templateType=APT');

        if (in_array($response->code, array(200, 201))) {
            return $response->body;
        } else {
            throw new MKException('Could not get LP Express presets. Response ('.$response->code.'): '.$response->raw_body, $response->body->code);
        }
    }
     ...
     */
    public function addMetaData($request_body)
    {
        $request_body["app_info"] = array(
            "module" => "MakeCommerce",
