...
class Payment {
	...
	public static function check_payment( $settings = array() ) {
		...
		$returnUrl = $order->get_meta( '_makecommerce_order_received_url', true );
		if ( !$returnUrl ) {
		...
		if ( isset( $totalAmount ) && $totalAmount != $order->get_total() ) {
			...
			if ( $paymentStatus == 'COMPLETED' ) {
				$notes[] = sprintf('%s: %s.',
					__( 'Order total and transaction amount did not match! Please contact us at', 'wc_makecommerce_domain' ),
					__( 'support@maksekeskus.ee', 'wc_makecommerce_domain' )
				);
				$notes[] = sprintf('%s: %s.',
				...
				// Empty the cart so the customer is not tempted to pay again
				$woocommerce->cart->empty_cart();

				return $returnUrl;
			}

			// If the payment was not complete then send the customer back to checkout
        ...
        if ( class_exists( OrderUtil::class )
            ...
            $results = $wpdb->get_results(
                $wpdb->prepare("
                    ...
                    INNER JOIN {$wpdb->prefix}wc_orders_meta AS meta ON meta.order_id = {$wpdb->prefix}wc_orders.id AND meta.meta_key = %s
